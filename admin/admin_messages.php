...
if (isset($_GET['user_id'])) {
    $userId = $_GET['user_id'];
    $stmt = $conn->prepare("SELECT m.*, u.first_name FROM messages m LEFT JOIN users u ON m.user_id = u.id WHERE m.user_id = ? ORDER BY m.created_at DESC");
    $stmt->execute([$userId]);
    ...
    <?php else: ?>
        <?php foreach (array_reverse($messages) as $msg): ?>
            <div class="message mb-2 flex <?php echo $msg['is_admin'] ? 'justify-end' : 'justify-start'; ?>">
                <div class="message-bubble <?php echo $msg['is_admin'] ? 'bg-green-200 text-white' : 'bg-gray-300 text-gray-800'; ?> rounded-lg p-2 max-w-xs">
                    <?php if ($msg['is_admin']): ?>
